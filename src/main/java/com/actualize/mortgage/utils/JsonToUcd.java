...
//	private static final XPath xPath = XPathFactory.newInstance().newXPath();
	...
	/**
     * inserts Deal Sets in MISMO XML
     * @param document Output XML file
     * @param element parent node of XML
     * @param jsonDocument Input JSON Object
     */
...
*/		insertTermsOfLoan(document, insertLevels(document, element, "TERMS_OF_LOAN"), jsonDocument.getTermsOfLoan());
	...
	*/
	...
	private void insertEscrowItem(Document document, Element element, EscrowItemModel escrowItem) {
		...
		List<MismoFeePaymentsModel> mismoFeePaymentsList = Convertor.toMismoFeePayments(paymentsModel, "ESCROW");
		if(mismoFeePaymentsList.size() > 0)
			insertEscrowItemPayments(document,  insertLevels(document, element, "ESCROW_ITEM_PAYMENTS"), mismoFeePaymentsList);
	}
	/**
	...
	private void insertEscrowItemPayments(Document document, Element element,
			List<MismoFeePaymentsModel> mismoFeePaymentsList) {
		for (MismoFeePaymentsModel mismoFeePayment : mismoFeePaymentsList)
			insertEscrowItemPayment(document, insertLevels(document, element, "ESCROW_ITEM_PAYMENT"), mismoFeePayment);
	}
	/**
	...
	private void insertEscrowItemPayment(Document document, Element element,
			MismoFeePaymentsModel mismoFeePayment) {
		insertData(document, element, "EscrowItemActualPaymentAmount",mismoFeePayment.getAmount());
		insertData(document, element, "EscrowItemPaymentPaidByType", mismoFeePayment.getPaidByType());
		insertData(document, element, "EscrowItemPaymentTimingType", mismoFeePayment.getClosingIndicator());
	}
	/**
	...
	*//**
	...
	/*private void insertProjectedPayments(Document document, Element element,
			ProjectedPaymentsDetails jsonDocument) {
	...
	*//**
	...
	private void insertCashToCloseItem(Document document, Element element,
			...
			//insertData(document, element, "IntegratedDisclosureCashToCloseItemAmountChangedIndicator", cashToClose.get);
	...
	*/
	...
	/**
	 * inserts construction in MISMO XML 
	 * @param document
	 * @param element
	 * @param construction
	 */
	...
	/**
	 * inserts BuydownRule in MISMO XML
	 * @param document
	 * @param element
	 * @param temporaryBuydown
	 */
	...
	/**
	 * inserts BuydownOccurences in MISMO XML
	 * @param document
	 * @param element
	 * @param temporaryBuydown
	 */
	private void insertBuydownOccurences(Document document, Element element, LoanTermsTemporaryBuydown temporaryBuydown) {
		//for (BuyDownOccurance buyDownOccurance : buyDownOccurances)
			insertBuydownOccurence(document ,insertLevels(document, element, "BUYDOWN_OCCURRENCES"), temporaryBuydown);//TODO Need To Add the Object
	}
	
	/**
	 * inserts BuydownOccurence in MISMO XML
	 * @param document
	 * @param element
	 * @param temporaryBuydown
	 */
    ...
    /**
     * inserts closingInformation in MISMO XML 
     * @param document
     * @param element
     * @param jsonDocument
     */
	...
	private void insertProrationItems(Document document, Element element, List<ProrationModel> prorationItems) {
		for (ProrationModel prorationItem : prorationItems)
			insertProrationItem(document, insertLevels(document, element, "PRORATION_ITEM"), prorationItem);//TODO Need To Add the Object
	}
	...
	*/
     ...
     * @param jsonDocument Input JSON Object
     */
	private void insertIndividual(Document document, Element element, ClosingDisclosure jsonDocument) {
	...
	*//**
	...
	private void insertPrincipalAndInterestPaymentPerChangeAdjustmentRules(Document document, Element element,
		...
		if("First".equalsIgnoreCase(principalAndInterestPaymentAdjustment.getFirstAdjustmentRuleType()))
			insertPrincipalAndInterestPaymentPerChangeAdjustmentRule(document,insertLevels(document, element, "PRINCIPAL_AND_INTEREST_PAYMENT_PER_CHANGE_ADJUSTMENT_RULE"),principalAndInterestPaymentAdjustment);
	...
	private void insertInterestRatePerChangeAdjustmentRules(Document document, Element element,
		...
		if("First".equalsIgnoreCase(interestRateAdjustment.getFirstAdjustmentRule()))
			insertInterestRatePerChangeAdjustmentRule(document ,insertLevels(document, element, "INTEREST_RATE_PER_CHANGE_ADJUSTMENT_RULE"), interestRateAdjustment);
	...
	*//**
	...
	private void insertParties(Document document, Element element, ClosingDisclosure jsonDocument) {
		List<Borrower> borrowers = jsonDocument.gettr;
